...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        proposals, scores_post_nms = utils.batch_slice(
            ...
            pooled.append(
                tf.cast(
                    tf.image.crop_and_resize(
                        tf.cast(feature_maps[i], "float32"),
                        tf.cast(level_boxes, "float32"),
                        box_indices,
                        self.pool_shape,
                        method="bilinear",
                    ),
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    negative_count = (
        tf.cast(r * tf.cast(positive_count, tensor_ftype), tf.int32) - positive_count
    )
    ...
    masks = tf.image.crop_and_resize(
        tf.cast(roi_masks, "float32"),
        tf.cast(boxes, "float32"),
        box_ids,
        config.MASK_SHAPE,
    )
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        if config.DETECTION_SCORE_RATIO:
            conf_keep = tf.compat.v1.where(
                class_ratio_scores > config.DETECTION_MIN_CONFIDENCE
            )[:, 0]
        else:
            conf_keep = tf.compat.v1.where(
    ...
    if config.DETECTION_SCORE_RATIO:
        ...
        num_keep = tf.minimum(tf.shape(input=class_ratio_scores_keep)[0], roi_count)
...
def fpn_classifier_graph(
    ...
    x = KL.TimeDistributed(
        KL.Conv2D(1024, (pool_size, pool_size), padding="valid"),
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    loss = K.switch(
        tf.size(input=loss) > 0, K.mean(loss), tf.constant(0.0, dtype=tensor_dtype)
    )
    return loss
...
def mrcnn_one_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    # Reshape for simplicity. Merge first two dimensions into one.
    target_class_ids = K.reshape(target_class_ids, (-1,))
    target_class_ids = tf.clip_by_value(
        target_class_ids, clip_value_min=0, clip_value_max=1
    )
    mask_shape = tf.shape(input=target_masks)
    target_masks = K.reshape(target_masks, (-1, mask_shape[2], mask_shape[3]))
    pred_shape = tf.shape(input=pred_masks)
...
def load_image_gt(dataset, config, image_id, augmentation=None, use_mini_mask=False):
    ...
    ZOOM_IN = (
        config.USE_OBJECT_ZOOM
        and min_img_dim > config.IMAGE_MAX_DIM
        and np.random.random() < config.ZOOM_IN_FREQ
    )
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    bboxes = np.zeros(
        (config.TRAIN_ROIS_PER_IMAGE, config.NUM_CLASSES, 4), dtype=tensor_dtype
    )
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = np.round(
                skimage.transform.resize(
                    class_mask, (gt_h, gt_w), order=1, mode="constant"
                )
            ).astype(bool)
    ...
    while True:
        try:
            ...
            if b == 0:
                ...
                batch_images = np.zeros((batch_size,) + image.shape, dtype=tensor_dtype)
...
class MaskRCNN:
    ...
    def build(self, mode, config):
        ...
        _, mrcnn_class, mrcnn_bbox = fpn_classifier_graph(
            rpn_rois,
            mrcnn_feature_maps,
            input_image_meta,
            config.POOL_SIZE,
    ...
    def load_weights(
        self, weights_in_path, weights_out_path, by_name=False, exclude=None
    ):
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [
            ...
            / tf.cast(tf.size(input=w), tensor_dtype)
    ...
    def unmold_detections(self, detections, original_image_shape, image_shape, window):
        ...
        #     if full_masks
        #     else np.empty(masks.shape[1:3] + (0,))
        # )
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, _, _, _, _ = self.keras_model.predict(
...
def norm_boxes_graph(boxes, shape):
    ...
    shift = tf.constant([0.0, 0.0, 1.0, 1.0], dtype=tensor_dtype)
